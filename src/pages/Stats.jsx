...
function Stats() {
  ...
  console.log(stats.usersDetails);
  return (
    <div className=''>
      {stats.loader && <Loader />}
      {stats.loader === false
        && <Container>
          <div className='w-1/3 h-[75vh] p-4 m-4 flex flex-col justify-evenly'>
            <h1 className='text-4xl font-semibold'>Hello, <h1 className='font-thin italic'>{stats.username}</h1></h1>
            <div className='flex justify-between'>
              <div className=''>
                <p className='text-2xl'>Registered Email</p>
                <p className='text-2xl'>{stats.email}</p>
              </div>
          ...
          <div className='w-2/3 h-[75vh]'>
            <div className='flex justify-between mx-6'>
              <p className='text-4xl font-semibold m-4'>Blogs: {stats.blogCount}</p>
              <p className='text-4xl font-semibold m-4'>Users: {stats.totalUsers}</p>
            </div>
            <div className='grid grid-cols-2 gap-8 overflow-scroll overflow-x-hidden scroll-smooth' style={{ 'height': 'calc(100vh - 40vh)' }}>
              {stats.usersDetails.map((user, index) => {
                return (
